...
const BASE_URL = "https://d3ad2g8hyy43zt.cloudfront.net";

// 🔐 helper to sign URLs
async function getSignedUrl(path: string) {
  const res = await fetch(
    `/api/cloudfront-signedurl?video=${encodeURIComponent(path)}`
  );
  return res.json();
}
    ...
    async function fetchVideos() {
      ...
      const formattedVideos = await Promise.all(
        videosArray.map(async (video: any) => {
          const cleanPath = video.filename.replace(BASE_URL, "");

          // 🔐 SIGN VIDEO URL
          const videoSigned = await getSignedUrl(cleanPath);

          // 🔐 BUILD THUMB PATH
          const folderPath =
            cleanPath.replace(/master\.m3u8$/, "").replace(/index\.m3u8$/, "");

          const thumbPath = `${folderPath}thumbnail.jpg`;

          // 🔐 SIGN THUMBNAIL URL
          const thumbSigned = await getSignedUrl(thumbPath);
          ...
          return {
            ...
            // 🔥 signed video URL
            masterUrl: videoSigned.url,

            // 🔥 signed thumbnail URL
            thumbnail: thumbSigned.url,
