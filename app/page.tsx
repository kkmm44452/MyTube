...
//               const cloudfrontBase = "https://d3ad2g8hyy43zt.cloudfront.net";

// <VideoPlayer
//   videoPath={`/api/cloudfront-playlist?video=${encodeURIComponent(
//     video.masterUrl.replace(cloudfrontBase, "")
//   )}`}
// />
  ...
  useEffect(() => {
    async function fetchVideos() {
      ...
      const formatted = await Promise.all(
        videosArray.map(async (video: any) => {
          // 🔥 ASK BACKEND FOR PLAYLIST URL
          const res = await fetch(
            `/api/video-playlist?video=${encodeURIComponent(video.filename)}`
          );
          ...
          return {
            ...
            title: video.title,
            description: video.description,
            playlistUrl, // ✅ CLEAN BACKEND HANDLED
            thumbnail:
              video.thumbnail ||
              `https://d3ad2g8hyy43zt.cloudfront.net${video.filename.replace(
                "master.m3u8",
